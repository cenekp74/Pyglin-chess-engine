            ...
            for wPieceSq in board.pieces(chess.PIECE_SYMBOLS.index(piecetype), 1):
                ...
                if piecetype == 'p':
                    ...
                    if chess.square_distance(wPieceSq, whiteKingSq) <= 2:
                        eval += 7
                    elif chess.square_distance(wPieceSq, whiteKingSq) <= 2:
                        eval += 2
            for bPieceSq in board.pieces(chess.PIECE_SYMBOLS.index(piecetype), 0):
                ...
                if piecetype == 'p':
                    ...
                    if chess.square_distance(bPieceSq, blackKingSq) <= 2:
                        eval -= 7
                    elif chess.square_distance(bPieceSq, blackKingSq) <= 2:
                        eval -= 2
    else:
        for piecetype, value in PIECES_VAL.items():
            for wPieceSq in board.pieces(chess.PIECE_SYMBOLS.index(piecetype), 1):
                ...
                if piecetype == 'p':
                    ...
                    if pawnsW[file] > 1:
                        eval -= 22
                    if chess.square_distance(wPieceSq, whiteKingSq) <= 1:
                        eval += 9
            for bPieceSq in board.pieces(chess.PIECE_SYMBOLS.index(piecetype), 0):
                ...
                if piecetype == 'p':
                    ...
                    if pawnsB[file] > 1:
                        eval += 22
                    if chess.square_distance(bPieceSq, blackKingSq) <= 1:
                        eval -= 9
...
def quiesce(board, alpha, beta, root):
    ...
    for move in sort_moves(board):
        if board.gives_check(move):
            ...
            if PIECES_VAL[board.piece_at(move.to_square).symbol().lower()] - PIECES_VAL[board.piece_at(move.from_square).symbol().lower()] > -200 or not board.is_attacked_by(not board.turn, move.to_square):
                board.push(move)
                score = -quiesce(board, -beta, -alpha, False)
                board.pop()
                if score >= beta:
...
def sort_moves(board):
    ...
    for move in remaining:
        if (board.is_capture(move) and not board.is_en_passant(move)):
            if (PIECES_VAL[board.piece_at(move.to_square).symbol().lower()] - PIECES_VAL[board.piece_at(move.from_square).symbol().lower()]) > 10:
                remaining.remove(move)
                yield move
...
def move_low_time():
    if amount_of_pieces(board) < 8:
        response = requests.get(f"http://tablebase.lichess.ovh/standard?fen={board.fen()}")
        json = response.json()
        if len(json['moves']) > 0:
            ...
            return move
    def pvsFast(board, alpha, beta, depthLeft):
        if board.is_checkmate():
            return 10000, None
        if board.is_stalemate() or board.is_repetition() or board.is_fifty_moves():
            return 0, None
        best2 = None
        if depthLeft == 0:
            return -evaluate(board,  alpha, beta, True), None
        best = None
        for index, move in enumerate(sort_moves(board)):
            board.push(move)
            score, _best2 = pvs(board, -beta, -score, depthLeft-1)
            board.pop()
            if score > alpha:
                alpha = score
                best = move
...
def is_endgame(board):
    ...
    for piecetype, value in PIECES_VAL.items():
        ...
        for bPieceSq in board.pieces(chess.PIECE_SYMBOLS.index(piecetype), 0):
            ...
            if piecetype == 'q':
                materialB += 100
    if materialW < 700: return True
    if materialB < 700: return True
...
def amount_of_pieces(board):
    pieces = 0
    for piecetype, _value in PIECES_VAL.items():
        for wPieceSq in board.pieces(chess.PIECE_SYMBOLS.index(piecetype), 1):
            pieces += 1
        for bPieceSq in board.pieces(chess.PIECE_SYMBOLS.index(piecetype), 0):
            pieces += 1
...
def pvs(board, alpha, beta, depthLeft):
    ...
    if amount_of_pieces(board) < 8:
        ...
        if len(json['moves']) > 0:
            moveUci = (json['moves'][0])['uci']
            move = chess.Move.from_uci(moveUci)
            if json['category'] == 'win':
                return -10000, move, None
            elif json['category'] == 'draw':
                return 0, move, None
    best2 = None
    if depthLeft == 0:
...
def zws(board, beta, depthLeft):
    ...
    #         move = chess.Move.from_uci(moveUci)
    #         if json['category'] == 'win':
    #             return -10000  + depthLeft*10
    #         elif json['category'] == 'loss':
    #             return 10000 - depthLeft*10
    #         elif json['category'] == 'draw':
    #             return 0
    if depthLeft == 0:
...
if __name__ == '__main__':
    board = chess.Board('8/3Q4/k1PN4/6p1/1P2K1P1/8/P5P1/8 w - - 99 106')
    start = time.time()
